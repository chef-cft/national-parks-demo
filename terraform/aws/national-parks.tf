resource "aws_instance" "permanent_peer" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sleep ${var.sleep}",
      "sudo mkdir -p /hab/user/${var.infra-package}/config /hab/user/${var.inspec-package}/config",
      ...
      "sudo cp /home/${var.aws_ami_user}/audit_linux.toml /hab/user/${var.inspec-package}/config/user.toml",
      "sudo cp /home/${var.aws_ami_user}/config_linux.toml /hab/user/${var.infra-package}/config/user.toml",
      "sudo hab svc load ${var.infra-origin}/${var.infra-package} --group ${var.group} --strategy at-once --channel stable",
      "sudo hab svc load ${var.inspec-origin}/${var.inspec-package} --group ${var.group} --strategy at-once --channel stable",  
    ]
...
resource "aws_instance" "mongodb" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo systemctl enable hab-sup",
      "sleep ${var.sleep}",
      "sudo mkdir -p /hab/user/mongodb/config /hab/user/${var.infra-package}/config /hab/user/${var.inspec-package}/config",
      "sudo chown hab:hab -R /hab/user",
      "sudo /sbin/sysctl -w net.ipv4.conf.all.accept_source_route=0",
      "sudo /sbin/sysctl -w net.ipv4.conf.default.accept_source_route=0",
      "sudo /sbin/sysctl -w net.ipv4.conf.default.accept_redirects=0",
...
resource "aws_instance" "national_parks" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo systemctl enable hab-sup",
      "sleep ${var.sleep}",
      "sudo mkdir -p /hab/user/haproxy/config /hab/user/${var.infra-package}/config /hab/user/${var.inspec-package}/config",
      "sudo chown hab:hab -R /hab/user",
      "sudo /sbin/sysctl -w net.ipv4.conf.all.accept_source_route=0",
      "sudo /sbin/sysctl -w net.ipv4.conf.default.accept_source_route=0",
      "sudo /sbin/sysctl -w net.ipv4.conf.default.accept_redirects=0",
